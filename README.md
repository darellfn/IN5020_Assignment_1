...
To enable client cache, use the flags: `-cc` `--client-cache`

To set the cache method, use the flags: `-fifo` `-oldest`

These flags are only valid if a client-cache-enabling flag precedes them.

To set the delay time (in milliseconds) use the flags: `-20` `-50`
